...
echo "Enabling OS Vulnerability Scanning in \"BASIC\" mode on $CLUSTER_NAME"
gcloud container clusters update $CLUSTER_NAME \
    --location=$REGION \
...
echo ""
echo "Checking if there are clusters added to a fleet...If so, remove from fleet and disable the Anthos API to prevent charges"
echo ""
